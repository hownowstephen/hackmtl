...
class YellowAPI(object):
    ...
    def find_business(self, what, where, uid, page=None, page_len=None, sflag=None, lang=None):
        """
        Perform the FindBusiness call.
        """
        url = self._build_url('FindBusiness', what=what, where=where, UID=uid, pg=page, pgLen=page_len, sflag=sflag, lang=lang)
        return self._perform_request(url)

    def get_business_details(self, prov, business_name, listing_id, uid, city=None, lang=None):
        """
        Perform the GetBusinessDetails call.
        """
        kws = {'prov': prov, 'bus-name': business_name, 'listingId': listing_id, 'city': city, 'lang': lang, 'UID': uid}
